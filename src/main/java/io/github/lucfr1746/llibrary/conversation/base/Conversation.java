package io.github.lucfr1746.llibrary.conversation.base;

import io.github.lucfr1746.llibrary.conversation.base.question.ChainedQuestion;
import io.github.lucfr1746.llibrary.conversation.base.question.InputValidator;
import io.github.lucfr1746.llibrary.conversation.base.question.Question;
...
public final class Conversation<MessageType, SenderType extends ConversationPartner<MessageType>> {
    ...
    public void acceptInput(MessageType message) {
                ...
                if (validationResult.shallAskQuestionAgain()) {
                    ...
                    doneHandler.accept(
                            ConversationContext.of(
                                    conversationPartner,
                                    inputs,
                                    ConversationContext.EndState.INPUT_VALIDATION_HANDLER_FAIL,
        ...
        Question<MessageType, SenderType> nextQuestion =
                this.questions.getNextQuestion(
                        new ChainedQuestion.ComputeContext<>(this.lastQuestion, message, this.conversationPartner));
        if (nextQuestion == null) {
            ended = true;
            conversationManager.unregisterConversation(conversationPartner.getUniqueIdentifier());
            doneHandler.accept(
                    ConversationContext.of(conversationPartner, inputs, ConversationContext.EndState.SUCCESS, null));
        } else {
            ...
            lastTimeoutTask =
                    timeoutScheduler.schedule(
                            () -> {
                                doneHandler.accept(
                                        ConversationContext.of(
                                                conversationPartner, inputs, ConversationContext.EndState.TIMEOUT, question.getIdentifier()));
    ...
    void callDoneState(ConversationContext.EndState endState) {
        if (ended) {
            return;
        }
        ended = true;
